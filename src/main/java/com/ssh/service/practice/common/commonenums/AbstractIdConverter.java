...
public abstract class AbstractIdConverter<T extends CommonEnum> extends BaseTypeHandler<T> implements AttributeConverter<T, Integer> {
	...
	public AbstractIdConverter(Class<T> clazz) {
		this.clazz = clazz;
		try {
			...
			logger.error("AbstractIdConverter初始化异常", e);
		}
	}
