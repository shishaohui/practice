...
public class CfcaUtils {
    ...
    private static PrivateKey getPrivateKey(String path, String pwd) throws Exception {
        ...
        System.out.println("rsaPrivateKey:" + ((RSAPrivateCrtKeyImpl) priKey).getPrivateExponent());
    ...
    public static void main(String[] args) throws Exception {
        PrivateKey privateKey = getPrivateKey(Constant.MEM_PFX_FILE_PATH, Constant.MEM_PFX_FILE_PASSWORD);

        String rsaPrivateKey = getrsaPrivateKey(Constant.MEM_PFX_FILE_PATH, Constant.MEM_PFX_FILE_PASSWORD);

        Cipher cipher = Cipher.getInstance("RSA","BC");
        cipher.init(Cipher.ENCRYPT_MODE,privateKey);
        byte[] bytes = cipher.doFinal(rsaPrivateKey.getBytes());
        String base64String = Base64.encodeBase64String(bytes);

    }


    private static String getrsaPrivateKey(String path, String pwd) throws Exception {
        ...
        System.out.println("is key entry=" + ks.isKeyEntry(keyAlias));
        RSAPrivateCrtKeyImpl priKey = (RSAPrivateCrtKeyImpl) ks.getKey(keyAlias, pwd.toCharArray());
        return String.valueOf(priKey.getPrivateExponent());
